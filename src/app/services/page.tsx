...
import { fetchAllServices, Service, ServiceData } from '@/app/utils/fetch_services';
...
export default function ServicesPage() {
  ...
  const validTypes = ['accommodations', 'cars', 'restaurants', 'activities'] as const;
  type ValidType = typeof validTypes[number];
  const type = typeParam && validTypes.includes(typeParam as ValidType) ? typeParam as keyof ServiceData : null;
  ...
  useEffect(() => {
    const loadServices = async () => {
      try {
        ...
        if (!type || !['accommodations', 'cars', 'restaurants', 'activities'].includes(type)) {
          router.push('/404');
          return;
        }
  ...
  const filteredServices = services.filter(service => {
    ...
    if (filters.minPrice !== '' && type) {
      ...
      if (type === 'accommodations' && 'rooms' in service) {
        const minRoomPrice = Math.min(...service.rooms.map(r => r.pricePerNight));
    ...
    if (filters.maxPrice !== '' && type) {
      ...
      if (type === 'accommodations' && 'rooms' in service) {
        const maxRoomPrice = Math.max(...service.rooms.map(r => r.pricePerNight));
  ...
  function getServicePrice(service: Service, type: string | null): number {
    ...
    switch (type) {
      case 'accommodations':
        return 'rooms' in service ? Math.min(...service.rooms.map(r => r.pricePerNight)) : 0;
      case 'cars':
  ...
  const getTypeLabel = (type: keyof ServiceData | null) => {
    switch (type) {
      case 'accommodations': return 'Places to Stay';
  ...
  return (
    <div>
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 mt-16 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold text-gray-900 mb-8">{getTypeLabel(type)}</h1>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 mb-8 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">💰 Min Price (MAD)</label>
                <input
                  type="number"
                  name="minPrice"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">💰 Max Price (MAD)</label>
                <input
                  type="number"
                  name="maxPrice"
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <option value="asc">
                      {filters.sortBy === 'price' ? 'Lowest to Highest' : 
                       filters.sortBy === 'rating' ? 'Lowest to Highest' : 
                       'A to Z'}
                    </option>
                    <option value="desc">
                      {filters.sortBy === 'price' ? 'Highest to Lowest' : 
                       filters.sortBy === 'rating' ? 'Highest to Lowest' : 
                       'Z to A'}
                    </option>
                  </select>
                </div>
          ...
          <div className="mb-6 flex justify-between items-center">
            <p className="text-sm text-gray-600">
              Showing <span className="font-semibold">{filteredServices.length}</span> of <span className="font-semibold">{services.length}</span> {filteredServices.length === 1 ? 'result' : 'results'}
            </p>
          </div>
          ...
          {filteredServices.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredServices.map((service) => (
                <Link 
                  ...
                  className="group"
                >
                  <div className="bg-white overflow-hidden shadow-lg rounded-lg hover:shadow-xl transition-all duration-300 h-full flex flex-col border border-gray-200 hover:border-blue-300">
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <div className="text-lg font-bold text-blue-600">
                          {type === 'accommodations' && 'rooms' in service 
                            ? `From ${Math.min(...service.rooms.map(r => r.pricePerNight))} MAD`
                        ...
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {type ? `${type.charAt(0).toUpperCase()}${type.slice(1, -1)}` : 'Service'}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              {filteredServices.map((service) => (
                ...
                >
                  <div className="bg-white overflow-hidden shadow-xl rounded-xl hover:shadow-2xl transition-all duration-300 border border-gray-100 hover:border-gold-300">
                    <div className="flex flex-col md:flex-row">
                      {service.images && service.images[0] && (
                        <div className="md:w-80 h-60 md:h-auto flex-shrink-0 overflow-hidden relative">
                          <img
                            src={service.images[0]}
                            alt={service.name}
                          ...
                          <div className="absolute top-4 left-4">
                            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-600 bg-opacity-90 text-white backdrop-blur-sm">
                              {type ? `${type.charAt(0).toUpperCase()}${type.slice(1, -1)}` : 'Service'}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {type === 'accommodations' && 'rooms' in service && (
                                <span className="px-3 py-1 bg-indigo-50 text-indigo-700 rounded-full text-xs font-medium inline-flex items-center">
                                  <Bed className="w-3.5 h-3.5 mr-1" strokeWidth={2} />
                              ...
                              <span className="px-3 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-medium inline-flex items-center">
                                ...
                                {('address' in service ? (service as { address?: { city?: string } }).address?.city : null) || 'Location Available'}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                          <div>
                            <div className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-1">Starting From</div>
                            <div className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">
                              {type === 'accommodations' && 'rooms' in service 
                                ? `${Math.min(...service.rooms.map(r => r.pricePerNight))} MAD`
                                : type === 'cars' && 'pricePerDay' in service
                                ? `${service.pricePerDay} MAD/day`
                                : type === 'restaurants' && 'minPrice' in service
